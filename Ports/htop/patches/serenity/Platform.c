...
int Platform_getUptime(void) {
   /* Use CLOCK_MONOTONIC as an approximation for system uptime */
   struct timespec ts;
...
void Platform_getLoadAverage(double* one, double* five, double* fifteen) {
   /* SerenityOS does not expose load averages */
   *one = 0.0;
   *five = 0.0;
   *fifteen = 0.0;
}
...
double Platform_setCPUValues(Meter* this, unsigned int cpu) {
   ...
   /* cpu==0 in htop means aggregate; cpus array index 0 is also aggregate */
...
void Platform_setMemoryValues(Meter* this) {
   ...
   this->values[MEMORY_METER_SHARED]  = 0;
   this->values[MEMORY_METER_BUFFERS] = 0;
   this->values[MEMORY_METER_CACHE]   = host->cachedMem;
}

void Platform_setSwapValues(Meter* this) {
   /* SerenityOS has no swap */
   this->total = 0;
...
void Platform_getFileDescriptors(double* used, double* max) {
   ...
   if (getrlimit(RLIMIT_NOFILE, &rl) == 0) {
      ...
      *used = NAN; /* actual open FD count not easily available */
   } else {
      *used = NAN;
      *max  = NAN;
   }
