...
![Icon](/res/icons/16x16/app-settings.png) Users Settings
...
New users are automatically added to the `users`, `window`, `audio`, `lookup` and `phys` groups, so they can login to their own desktop environment. And their home directory is created with the standard subdirectories (`Desktop`, `Documents`, `Downloads`, `Music`, `Pictures`, `Videos`).
