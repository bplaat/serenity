...
ErrorOr<void> ViewWidget::setup()
{
    ...
    m_font_button = find_descendant_of_type_named<GUI::Button>("terminal_font_button");
    ...
    m_opacity = Config::read_i32("Terminal"sv, "Window"sv, "Opacity"sv, Terminal::default_opacity);
    ...
    m_initial_font_name = Config::read_string("Terminal"sv, "Text"sv, "Font"sv);
    if (m_initial_font_name.is_empty())
        m_font = Gfx::FontDatabase::the().default_fixed_width_font();
    else
        m_font = Gfx::FontDatabase::the().get_by_name(m_initial_font_name);
    m_original_font = m_font;
    m_font_label->set_text(m_font->human_readable_name());
    m_font_label->set_font(m_font);
    m_font_button->on_click = [this](auto) {
    ...
    m_use_default_font_checkbox->on_checked = [this](bool use_default_font) {
        if (use_default_font) {
            ...
            m_font = m_initial_font_name.is_empty()
                ? Gfx::FontDatabase::the().default_fixed_width_font()
                : Gfx::FontDatabase::the().get_by_name(m_initial_font_name);
        }
        set_modified(true);
    };
    ...
    m_use_default_font_checkbox->set_checked(m_font == Gfx::FontDatabase::the().default_fixed_width_font(), GUI::AllowCallback::No);
    m_font_selection->set_enabled(!m_use_default_font_checkbox->is_checked());

    m_cursor_shape = VT::TerminalWidget::parse_cursor_shape(Config::read_string("Terminal"sv, "Cursor"sv, "Shape"sv)).value_or(Terminal::default_cursor_shape);
    m_original_cursor_shape = m_cursor_shape;

    m_cursor_is_blinking_set = Config::read_bool("Terminal"sv, "Cursor"sv, "Blinking"sv, Terminal::default_cursor_blinking);
    ...
    m_cursor_blinking_checkbox->set_checked(Config::read_bool("Terminal"sv, "Cursor"sv, "Blinking"sv, Terminal::default_cursor_blinking), GUI::AllowCallback::No);
    ...
    m_max_history_size = Config::read_i32("Terminal"sv, "Terminal"sv, "MaxHistorySize"sv, Terminal::default_max_history_size);
    ...
    m_show_scrollbar = Config::read_bool("Terminal"sv, "Terminal"sv, "ShowScrollBar"sv, Terminal::default_show_scrollbar);
...
{
    m_opacity = Terminal::default_opacity;
    m_cursor_shape = Terminal::default_cursor_shape;
    m_cursor_is_blinking_set = Terminal::default_cursor_blinking;
    m_max_history_size = Terminal::default_max_history_size;
    m_show_scrollbar = Terminal::default_show_scrollbar;
    m_font = Gfx::FontDatabase::the().default_fixed_width_font();
