...
ErrorOr<void> UserDetailsWidget::initialize()
{
    ...
    m_change_password_button->on_click = [this](auto) {
        MUST(change_password());
    };

    m_full_name_textbox->on_change = [this] {
        if (!m_loading && on_modified)
            on_modified();
    };
...
void UserDetailsWidget::load_account(Core::Account const& account)
{
    m_account = account;
    m_loading = true;
...
ErrorOr<void> UserDetailsWidget::apply_changes()
{
    ...
    if (auto result = m_account->sync(); result.is_error()) {
        GUI::MessageBox::show(window(), "Failed to save changes."sv, "Error"sv, GUI::MessageBox::Type::Error);
        return result;
    }
