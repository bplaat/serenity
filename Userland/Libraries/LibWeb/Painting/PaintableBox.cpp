...
{
    if (scroll_bar_rect(ScrollDirection::Vertical).has_value())
        return true;
    if (scroll_bar_rect(ScrollDirection::Horizontal).has_value())
        return true;
    return false;
}
...
Optional<CSSPixelRect> PaintableBox::scroll_bar_rect(ScrollDirection direction) const
{
...
{
    auto bar_rect = scroll_bar_rect(direction);
...
Gfx::ScrollbarState PaintableBox::compute_scrollbar_state(ScrollDirection direction) const
{
    ...
    return Gfx::ScrollbarState {
        .dec_pressed = pressed == ScrollbarComponent::DecrementButton && hovered == ScrollbarComponent::DecrementButton && !is_at_min,
        .dec_hovered = hovered == ScrollbarComponent::DecrementButton,
        .inc_pressed = pressed == ScrollbarComponent::IncrementButton && hovered == ScrollbarComponent::IncrementButton && !is_at_max,
        .inc_hovered = hovered == ScrollbarComponent::IncrementButton,
...
void PaintableBox::paint(PaintContext& context, PaintPhase phase) const
{
    ...
    if (phase == PaintPhase::Overlay && scrollbar_width != CSS::ScrollbarWidth::None) {
#if defined(AK_OS_SERENITY)
        if (auto bar_rect = scroll_bar_rect(ScrollDirection::Horizontal); bar_rect.has_value()) {
        ...
        if (auto bar_rect = scroll_bar_rect(ScrollDirection::Vertical); bar_rect.has_value()) {
        ...
        if (scroll_bar_rect(ScrollDirection::Horizontal).has_value() && scroll_bar_rect(ScrollDirection::Vertical).has_value()) {
            auto padding_rect = absolute_padding_box_rect();
            if (is_viewport())
...
{
    if (auto bar_rect = scroll_bar_rect(ScrollDirection::Vertical); bar_rect.has_value() && bar_rect->contains(position))
        return callback(HitTestResult { const_cast<PaintableBox&>(*this) });
    if (auto bar_rect = scroll_bar_rect(ScrollDirection::Horizontal); bar_rect.has_value() && bar_rect->contains(position))
        return callback(HitTestResult { const_cast<PaintableBox&>(*this) });
