...
ErrorOr<void> ELFWidget::open_file(String const& path)
{
    ...
    elf_image.for_each_symbol([this](auto const& sym) {
        if (sym.type() == STB_LOCAL) {
            m_exports.append(MUST(String::from_deprecated_string(AK::demangle(sym.name()))));
        }
    });
    ...
    elf_image.for_each_symbol([this](auto const& sym) {
        if (sym.type() == STB_GLOBAL || sym.type() == STB_WEAK) {
            m_imports.append(MUST(String::from_deprecated_string(AK::demangle(sym.name()))));
        }
    });
